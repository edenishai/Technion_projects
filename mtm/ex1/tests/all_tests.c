...
bool testPQCopySingleElement() {
    ...
    PQElementPriority temp1 = pqGetFirst(new_pq);
    PQElementPriority temp2 = pqGetFirst(pq);
    ASSERT_TEST(compareIntsGeneric(temp1, temp2) == 0, destroy);
...
bool testPQChangePriorityAlsoChangesPositionInQueue() {
    ...
    ASSERT_TEST((*(int *) pqGetFirst(pq)), destroy);
...
bool testPQChangePriorityWOnlyChangesFirstOfMultipleSameElementsAndReinsertsIt() {
    ...
    int elem10 = 10;
    int prio10 = 10;
    pqInsert(pq, &elem10, &prio10);

    int new_prio = prio10;
    ASSERT_TEST(pqChangePriority(pq, &elem, &prio, &new_prio) == PQ_SUCCESS, destroy);

    int temp = (*(int *) pqGetFirst(pq));
    ASSERT_TEST(temp == elem10, destroy); // Original elem2
...
bool testPQGetNextIteratorStaysTheSameInMostFunction() {
    ...
    pqInsert(pq, &elem3, &elem3);
    int temp = (*(int *) pqGetFirst(pq));
    ASSERT_TEST(temp, destroy);
    temp = (*(int *) pqGetNext(pq));
    ASSERT_TEST(temp == 2, destroy);
